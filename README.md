...
- **Classification head** — an MLP with Dropout(0.3) and a sigmoid output produces the cancer
  probability `p ∈ [0, 1]`.
...
For 516 patients, the rounded hold-out count gives 361 training, 52 tuning,
and 103 test patients. The exact patient IDs are saved in `split_patient_ids.json`;
matching counts alone does not recover a historical split. If a final training
batch contains only one patient, it is merged into the preceding batch to support
BatchNorm without discarding any patients. Binary predictions use probability
...
model:
  ...
  num_heads: 8
  num_layers: 3
  dropout: 0.1  # Transformer; cfDNA MLP and classification head use 0.3
  cfdna_dim: 275
training:
  epochs: 100
  batch_size: 4
