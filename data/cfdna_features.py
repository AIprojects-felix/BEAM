...
feature vector obtained by concatenating five domain-specific modules:
    ...
    CNV      (44  dims)  Chromosomal-arm-level z-scores from 1-Mb CNV bins.
    FSR      (80  dims)  Fragment Size Ratio (5-Mb bins) after GC correction + PCA.
    Griffin  (100 dims)  Nucleosome-positioning coverage features over a panel
                         of 50 prostate-relevant transcription factors
                         (2 metrics per TF).
    MutCS    (30  dims)  Contributions retained from 79 COSMIC SBS signatures
                         by filtering mean activity in the training cohort.
    FragMA   (21  dims)  cfDNA 5'-end-motif features.

Total:                   275 dims.

PCA and data-dependent feature selection are fitted upstream on training
patients only and frozen for tuning, hold-out testing, and evaluation.
...
def build_cfdna_feature_vector(features: Dict[str, np.ndarray]) -> np.ndarray:
    ...
    Args:
        features: {'cnv': (44,), 'fsr': (80,), 'griffin': (100,), 'mutcs': (30,), 'fragma': (21,)}
