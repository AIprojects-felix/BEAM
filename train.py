...
from data.cfdna_scaling import scaler_to_state
...
def split_discovery_cohort(labels, test_size=0.2, val_size=0.125, seed=42):
    """Stratified split; round the hold-out count (516 -> 361/52/103).

    val_size is the tuning fraction of the remaining development cohort.
    Saved patient IDs, rather than these counts alone, identify a study split.
    """
    labels = np.asarray(labels)
    indices = list(range(len(labels)))
    test_count = int(round(len(labels) * test_size))
    train_idx, test_idx = train_test_split(
        indices, test_size=test_count, random_state=seed, stratify=labels,
    )
    train_idx, val_idx = train_test_split(
        train_idx, test_size=val_size, random_state=seed, stratify=labels[train_idx],
    )
...
def main(args):
    ...
    train_idx, val_idx, test_idx = split_discovery_cohort(
        labels, test_size=test_size, val_size=val_size, seed=seed,
    )
    logger.info(f'Split: train={len(train_idx)}, val={len(val_idx)}, test={len(test_idx)}')

    # Learn scaling only from the training partition; all subsets reuse it.
    scaler = dataset.fit_cfdna_scaler(train_idx)
    if scaler.n_features_in_ != cfdna_dim:
        raise ValueError('Training cfDNA feature dimension does not match model')
    scaler_state = scaler_to_state(scaler)
    split_ids = {
        name: [dataset.patient_ids[i] for i in idx]
        for name, idx in [('train', train_idx), ('tuning', val_idx), ('test', test_idx)]
    }
    with open(os.path.join(exp_dir, 'split_patient_ids.json'), 'w') as f:
        json.dump(split_ids, f, indent=2)

    train_subset = Subset(dataset, train_idx)
    # 361 cases with batch size 4 leaves a singleton, which BatchNorm1d cannot
    # train on. Merge it into the preceding batch so every patient is retained.
    train_batches = MergeSingletonBatchSampler(
        torch.utils.data.RandomSampler(train_subset), batch_size,
    )
    train_loader = DataLoader(train_subset, batch_sampler=train_batches,
