...
from data.cfdna_scaling import scaler_from_checkpoint, transform_cfdna
from models import BEAM
from utils import create_dirs, load_config


def load_patient(pdir: Path, target_shape, scaler):
    ...
    cfdna = transform_cfdna(np.load(pdir / 'cfdna_features.npy'), scaler)
    return mri, cfdna
...
def main(args):
    ...
    scaler = scaler_from_checkpoint(ckpt, cfdna_dim)
    ...
    if args.batch_mode:
        ...
        for pdir in patient_dirs:
            try:
                mri, cfdna = load_patient(pdir, target_shape, scaler)
        ...
        mri, cfdna = load_patient(args.mri_dir, target_shape, scaler)
        prob = infer(model, mri, cfdna, device)
